...
**What it's actually for (Adam's own framing, 2026-09-14): a betting app built to stay ahead of Polymarket — using AI to find smart strategy bets, not just to display numbers.** The goal is beating the market, so calibration and proven hit-rate are the core product, not polish on top of it. Any work here should be judged against "does this make his bets better," and the open calibration/backtest item below is therefore the highest-value thread, not a cleanup task.

Built via SuperGrok's "Build with Grok" (same tool used for [[TWI Sniper ORB]]) — separate from [[RetroVault (CeeloEdge)]], which is the Base44 version of a similar idea. Shows PolyEdge's own model probability against live Polymarket market price per game, flags large disagreements as possible edges.
...
## Live URL and the "Polymarket OFFLINE" diagnosis (2026-09-14)

Live app: `https://lotus-sky-slate-star.grok.me` (Grok-hosted). Adam surfaced it 2026-09-14. Its Live board showed **"Polymarket OFFLINE"** while **"ESPN POLLING"** was healthy, and the board read "No high-confidence opportunities. PolyEdge is passing" — which is NOT the model being disciplined, it's the model with no market data to compare against. The whole product premise (model price vs. market price) is dead while that feed is down.

**Diagnosed from this machine with real requests, ruling out each layer in order:**
1. **Polymarket itself is up.** `gamma-api.polymarket.com`, `clob.polymarket.com` and `polymarket.us` all returned HTTP 200 in well under a second.
2. **Not a CORS problem.** Both `gamma-api` and `clob` return `access-control-allow-origin: *`, so a browser-hosted app can call them directly.
3. **The Polymarket US API is key-gated in practice.** The SDK's README advertises "Public Endpoints (No Authentication)" for events/markets/order books, but real probing shows only the exact documented sample call, `GET api.polymarket.us/v1/markets?limit=1`, returns 200 — and it returns a canned demo payload (`"id":"1"`, a fixed 2025-11-02 NFL game). Every other form — no params, `limit=2`, `limit=10`, `offset=1`, and the `/v1/events`, `/v1/orderbook`, `/v1/prices` endpoints — returns `401 Missing required API key headers`. The unversioned `/markets` path 401s as well.

**Conclusion: PolyEdge has no valid Polymarket US credentials, and no code change alone will fix it.** Polymarket US uses Ed25519 signature auth (`keyId` + base64 Ed25519 `secretKey`), with keys generated by the account holder at `polymarket.us/developer` — something only Adam can do.

**Two real paths, and it's Adam's call which:**
- **Get Polymarket US keys** — required if he actually bets on the US-regulated venue, since only that API carries those markets and prices. **Security constraint: the Ed25519 secret key must never sit in browser-side code** — anyone opening devtools on a Grok-hosted page could read it. It needs to live in server-side env vars or behind a proxy.
- **Use the open global Polymarket API instead** (`gamma-api`/`clob`) — fully public, no key, CORS-open, confirmed working above. Free and immediate, but it carries the global market set, which is not identical to the US-regulated listings. Fine if the goal is market consensus as a reference price; wrong if he needs the exact book he's betting into.
