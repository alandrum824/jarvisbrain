...
4. If the push is rejected (remote has commits this session doesn't have — normal when another session pushed first): `git fetch origin`, review `git log HEAD..origin/main --oneline` to see what's incoming (it's expected multi-session work, not a red flag), then `git pull origin main --no-rebase` to merge.
