...
- [[PolyEdge (Grok Build)]] — Adam's other sports-edge app (SuperGrok build, separate from RetroVault). Found and fixed a real bug (model gave FCS teams sane-looking win probabilities against FBS Power opponents because ESPN's feed carries no explicit classification field). Confidence-score clustering across unrelated games still unresolved; The Odds API being wired in as a second market reference to separate real edges from model bugs.
