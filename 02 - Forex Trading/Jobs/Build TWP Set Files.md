...
- **This EA can hang the MT5 Strategy Tester indefinitely on certain grid configs** (2026-08-30) — confirmed via flat CPU usage over repeated checks (not just a slow run), on two different files with different grid ladders, no obvious common trigger (ladder depth didn't correlate — a 10-level ladder ran clean, a 3-level one hung). Compiled-only, can't diagnose from source. If a backtest looks stuck, check `Get-Process metatester64 -ErrorAction SilentlyContinue | Select CPU` twice a few seconds apart — climbing means it's working, identical means it's dead. Safe to force-kill (tester/demo only).
