...
## 2026-08-30 — SL=0.0 question partially answered empirically, balance updated

Never confirmed with TradeWithPat docs/support directly, but real backtest evidence gathered tonight (see [[TWP24 EURUSD Sets]] for the full session): every config tested with `iSL=0`/`iSLMode=1` (ATR mode) produced *bounded* losses — largest single loss across all those runs was -$42.97, never a runaway/uncapped loss that would indicate "no stop is actually placed." Not a certain answer (still no vendor confirmation), but real evidence leans toward "0.0 means use the ATR-derived default," not "no stop loss." Separately, a different file (`EURUSD_M5_NYSE_NO_GRID.set`) used an explicit non-zero `iSL=2.2` and also ran clean — so both modes appear to place a real stop in practice.

**Balance re-confirmed 2026-08-30 via live screenshot: ~$414–$420** (up from the $391.39 recorded 2026-08-21), Free Margin = Equity = Balance (no open positions at time of screenshot). Account is genuinely active and healthy.

**Real trading history reviewed tonight** (MT5 mobile screenshots, multiple accounts/dates): confirms Adam has run this EA with real multi-symbol grid positions (GBPJPY, CHFJPY, EURUSD, XAUUSD, even BTCUSD) at lot sizes from 0.08 up to 2.8 depending on account size — consistent with the "size the file to the account, don't reuse a bigger account's lot size" lesson already documented in [[TWP24 EURUSD Sets]].

**New real bug found tonight:** the compiled EA can hang the MT5 Strategy Tester indefinitely on certain grid-ladder configs (confirmed via flat CPU, not just slow) — not consistently tied to ladder depth. Not diagnosable further without source (compiled-only, no `.mq5` available for this EA — unlike [[TWI Sniper ORB]], which Adam controls the source of). Safe to force-kill if a backtest looks stuck; no real capital at risk in the tester itself.
