...
## Gotcha banked 2026-08-25 — Pine Editor connector tools went fully dark; real fallback found (clipboard-paste injection)

Building [[TWI Weekly Bias]], every high-level Pine tool — `pine_open`, `pine_set_source`, `pine_smart_compile`, `pine_get_source`, `pine_get_errors` — failed with **"Could not open Pine Editor"** (once, more specifically: **"...or Monaco not found in React fiber tree"**), across 10+ retries. Not the usual 1-3-retry flakiness banked above — this was a real, persistent break. Root cause: the tool locates the Monaco editor instance by walking the DOM element's React fiber (`__reactFiber$...` keys); on this session/build, `Object.keys()` on the `.monaco-editor` element found **zero** fiber keys — the tool's detection method doesn't match this app's actual internals right now.

**Real, verified-working fallback (bypasses the broken tool path entirely):**
1. Confirm the Pine Editor is genuinely open and on the right script (visually, via `capture_screenshot` — the title bar and "Add to chart"/"Saved" button state are reliable even when the connector tools aren't).
2. Duplicate a safe base script via the UI itself: click the script-name dropdown (`ui_click by text` on the current title) → **Make a copy…** → `ui_mouse_click` at the menu-item's real coordinates from a fresh `ui_find_element` call (not `ui_click by text` — that failed here) → clear the name field (`ui_keyboard Ctrl+A` then `ui_type_text`) → click **Make copy**. Confirm via `pine_list_scripts` (script count +1, new `id`).
3. **To inject real source when `pine_set_source` is broken:** find Monaco's hidden input via `ui_evaluate`: `document.querySelector('.monaco-editor textarea')` — this element's `.value` genuinely holds the live buffer and is a reliable read-check. To **write**, don't set `.value` directly (Monaco doesn't sync from that) — instead click into the editor, `ui_keyboard Ctrl+A` to select all, then dispatch a **synthetic paste event**: build a `DataTransfer`, `setData('text/plain', source)`, wrap in `new ClipboardEvent('paste', {clipboardData: dt, bubbles: true, cancelable: true})`, and `dispatchEvent` it on the textarea via `ui_evaluate`. Monaco's real paste handler picks it up and updates the model correctly — verified by re-reading `.view-line` DOM text afterward.
4. **Base64-encode the source before embedding it in the `ui_evaluate` JS string** (`atob(...)` inside the JS to decode) — avoids all quote/backslash/template-literal escaping headaches for a multi-hundred-line script passed through a tool-call JSON parameter.
5. Save with `ui_keyboard Ctrl+S` (works fine, not part of the broken path), then click **Add to chart** (via `ui_find_element`-sourced coordinates) to compile + attach in one step — a clean attach with no error dialog is itself the compile-success signal when `pine_get_errors` is unreachable. Confirm with `chart_get_state` (new study ID appears) and `data_get_pine_tables`/`data_get_pine_labels` to verify it's producing real, sane output on real data — don't just trust "it attached."

This whole path (copy → paste-inject → save → add to chart → verify via data tools) is a complete, real substitute for the normal `pine_open`/`pine_set_source`/`pine_smart_compile` flow when that flow is broken. Try the normal flow first every time (it's faster when it works) — fall back to this only after a genuine string of failures, not on the first miss.
