...
This vault lives at `C:\Users\aland\OneDrive\Desktop\jarvis\Adam`. If you use Claude Desktop, claude.ai, or any AI other than Claude Code, you have to point it at this path (set it in your MCP / filesystem connector, and tell the AI "my vault is here"). An AI can't read or maintain a vault it can't find.

---

## Who I Am

I'm Adam Landrum, a forex trader, born May 9, 1985.

## Forex Trading (02 - Forex Trading)
...
Never ask Adam what the frontmatter values should be. Infer them.
...
- `02 - Forex Trading/*` → `forex-trading`
...
When Adam says something is done or asks to archive a note: (1) set its frontmatter `status: archived` and save; (2) move it to the Archive folder, same filename; (3) confirm what was archived and where. Always confirm before archiving. Never archive on your own initiative.
...
If today's note already exists from an earlier session, append a new session section (`## Session 2`, `## Evening Session`) and add a line to the Index block — don't overwrite. Timestamp each entry with Adam's local time.
...
**Then push to GitHub (Adam's standing instruction, 2026-08-31/09-01 — given independently in two sessions the same night).** Once the daily note is written and any other checkpoint edits are in, sync the vault to `origin/main`. Full method, including how to resolve a multi-session merge conflict, is in [[Vault GitHub Sync]] — don't duplicate the steps here, keep that note current instead. Do this every day the vault changed; don't let work sit unsynced overnight.
...
This is universal — every AI that reads this vault does it. Adam uses multiple AIs across multiple sessions, no single one sees everything, so each contributes what it knows and the daily note fills in over time. Don't make a production of it. Briefly say what you did and move on.

### Living Profile

This file is a living document. Update the profile sections as you learn new things about Adam through conversation. Updates happen silently and are logged in the daily note under "Profile Updates."

**You can update:** Key People · How I Think · Health · Personal Interests · Beliefs · Daily Routine.
**You must NOT update:** Who I Am (basic bio — only Adam changes it) · the project sections · What's Active Right Now (lives in Active Priorities) · My Preferences for Working with AI · Vault Rules for AI.
