...
- [ ] Live account login failing ("invalid account", ending 8996721, AAAFxGlobal-5 Real) — creds or account number may be stale (forex-trading) — see [[TWP24 EURUSD Sets]].
- [ ] Sort out which TWP account is which — at least 3 distinct accounts now in play (broken login ending 8996721, config account 108.181.193.243, trade-history account D3101:81322) — needed before trusting any config/lot-sizing reference (forex-trading) — see [[TWP ORB EA Reference]].
...
- [ ] Brainstorm a revamp for mom's eBay store (EMarketingTX) before she shuts it down — sales dried up, no account access yet, still at the idea stage (personal) — see [[Mom's eBay Store (EMarketingTX)]].
...
- [ ] Get [[TWI Scalp Pro]] onto the second MT5 terminal, which lives on the machine Adam reaches via Remote Desktop (separate Jarvis session there) — handoff instructions are in the note; also still needs compiling and testing (forex-trading).
